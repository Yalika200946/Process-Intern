"""
Helper: builds notebooks/production/13_cleaning_plan_optimization.ipynb (moved
2026-07-17, was 16_cleaning_plan_optimization.ipynb; this builder script itself moved to
scripts/ 2026-07-19 so notebooks/ holds only .ipynb files) from cell definitions.
Run once: python scripts/_build_cleaning_plan_notebook.py
...
plt.tight_layout(); plt.savefig(REPO / 'figures' / 'fig_cleaning_plan.png', dpi=140, bbox_inches='tight'); plt.show()
""".strip("\n")))
...
- **online-cleaning capability มาจาก `src/domain/bypass.py` เท่านั้น** (parse ตรงจาก `list bypass Cleaning Heat
...
out_path = Path(__file__).resolve().parent.parent / 'notebooks' / 'production' / '13_cleaning_plan_optimization.ipynb'
