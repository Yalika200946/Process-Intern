...
def main():
    ...
    acceptance=json.loads((ROOT/"config/pilot_network_acceptance.json").read_text(encoding="utf-8"));main_metric=metrics[metrics.scope.eq("ALL_VALID")].iloc[0];mix_limit=acceptance["mix_temperature_closure"]
    mix_pass=(main_metric.MAE_C<=mix_limit["mae_limit_c"] and abs(main_metric.bias_C)<=mix_limit["absolute_bias_limit_c"] and main_metric.P90_absolute_error_C<=mix_limit["p90_absolute_error_limit_c"])
    intermediate=pilot_metrics[pilot_metrics.node.eq("E103_OUT/E104_IN")].iloc[0];outlet=pilot_metrics[pilot_metrics.node.eq("E104_OUT")].iloc[0]
    pilot_pass=(intermediate.mae_c<=acceptance["pilot_intermediate_node"]["mae_limit_c"] and abs(intermediate.bias_c)<=acceptance["pilot_intermediate_node"]["absolute_bias_limit_c"] and outlet.mae_c<=acceptance["pilot_branch_outlet"]["mae_limit_c"] and abs(outlet.bias_c)<=acceptance["pilot_branch_outlet"]["absolute_bias_limit_c"])
    metrics.loc[metrics.scope.eq("ALL_VALID"),"engineering_status"]="PASS_PROVISIONAL" if mix_pass else "FAIL"
    pilot_metrics["engineering_tolerance_c"]=pilot_metrics.node.map({"E103_OUT/E104_IN":acceptance["pilot_intermediate_node"]["mae_limit_c"],"E104_OUT":acceptance["pilot_branch_outlet"]["mae_limit_c"]})
    pilot_metrics["engineering_status"]=["PASS_PROVISIONAL" if row.mae_c<=row.engineering_tolerance_c and abs(row.bias_c)<=row.engineering_tolerance_c else "FAIL" for row in pilot_metrics.itertuples()]
    gates={"Gate A - Flow split":"PASS_PROVISIONAL" if flow.loc[flow.operating_state.eq("NORMAL"),"within_tolerance"].mean()>=.90 else "FAIL","Gate B - Mix-temperature closure":"PASS_PROVISIONAL" if mix_pass else "FAIL","Gate C - Temperature continuity":"PASS_PROVISIONAL" if cont.usable_for_network.all() else "FAIL","Gate D - Configuration response":"PASS_PROVISIONAL" if len(responses) else "BLOCKED","Gate E - Pilot propagation":"PASS_PROVISIONAL" if pilot_pass else "FAIL"}
    decision=evaluate_network_gates(gates);gate=pd.DataFrame([{"gate":k,"status":v,"basis":"Provisional engineering thresholds explicitly accepted for pilot scope on 2026-07-22.","approval_status":acceptance["approval_status"]} for k,v in gates.items()]);gate["network_status"]=decision["network_status"]
