...
def period_comparison(frame: pd.DataFrame, spec: dict) -> pd.DataFrame:
    ...
    for name,(a,b) in periods.items():
        ...
        rows.append({"hx_id":spec["hx_id"],"period":name,"start":a,"end":b,"valid_record_count":len(g),
                     ...
                     "median_relative_ua_empirical":g.relative_ua_empirical.median(),
                     "median_relative_performance_loss":g.relative_performance_loss.median(),
                     "empirical_reference_status":STATUS})
    return pd.DataFrame(rows)
...
def sensitivity(frame: pd.DataFrame,spec:dict,shifts:list[int])->pd.DataFrame:
    ...
    base=frame[(frame.timestamp>=start)&(frame.timestamp<=end)]
    if len(base) < MIN_REFERENCE_RECORDS:
        return pd.DataFrame([{"hx_id":spec["hx_id"],"status":"INSUFFICIENT_REFERENCE_DATA",
                              "empirical_reference_warning_code":"INSUFFICIENT_REFERENCE_DATA"}])
    base_refua=float(base.ua_value.median());base_relative=frame.ua_value/base_refua
    signed_shifts=sorted(set(shifts) | {-shift for shift in shifts})
    for ss in signed_shifts:
        for se in signed_shifts:
            a,b=start+pd.Timedelta(days=ss),end+pd.Timedelta(days=se);ref=frame[(frame.timestamp>=a)&(frame.timestamp<=b)]
            if b<a or len(ref)<MIN_REFERENCE_RECORDS:continue
            refua=float(ref.ua_value.median());relative=frame.ua_value/refua;loss=1-relative
            delta=(relative-base_relative).abs()
            rows.append({"hx_id":spec["hx_id"],"shift_start_days":ss,"shift_end_days":se,
                         "empirical_reference_start":a,"empirical_reference_end":b,
                         "empirical_reference_valid_records":len(ref),"reference_ua_empirical":refua,
                         "reference_ua_change_pct":100*(refua/base_refua-1),
                         "median_absolute_relative_ua_change":float(delta.median()),
                         "max_absolute_relative_ua_change":float(delta.max()),
                         "relative_ua_empirical_min":relative.min(),"relative_ua_empirical_max":relative.max(),
                         "relative_performance_loss_min":loss.min(),"relative_performance_loss_max":loss.max(),
                         "above_reference_pct":(relative>1).mean()*100,
                         "empirical_reference_status":STATUS,"cleaning_event_confirmed":False,
                         "clean_condition_confirmed":False,"empirical_reference_warning_code":""})
    return pd.DataFrame(rows)
...
def plot_outputs(frame,spec,throughput,periods,sens,out):
    ...
    axes[0].plot(frame.timestamp,frame.ua_value,lw=.7);axes[0].axhline(frame.reference_ua_empirical.iloc[0],color="green",ls="--",label="Empirical reference UA");axes[0].set_ylabel("UA (kW/K)");axes[0].legend()
    axes[1].plot(frame.timestamp,frame.relative_ua_empirical,lw=.7);axes[1].axhline(1,color="green",ls="--");axes[1].set_ylabel("Relative UA (-)")
    axes[2].plot(frame.timestamp,frame.relative_performance_loss,lw=.7);axes[2].axhline(0,color="green",ls="--");axes[2].fill_between(frame.timestamp,frame.relative_performance_loss,0,where=frame.relative_performance_loss<0,color="purple",alpha=.2,label="ABOVE_EMPIRICAL_REFERENCE");axes[2].set_ylabel("Relative performance loss (-)");axes[2].set_xlabel("Time (Asia/Bangkok)");axes[2].legend()
    ...
    fig,ax=plt.subplots(figsize=(8,5));groups=[throughput.loc[throughput.throughput_band==b,"relative_ua_empirical"] for b in ["LOW","MEDIUM","HIGH"]];ax.boxplot(groups,tick_labels=["LOW","MEDIUM","HIGH"],showfliers=False);ax.axhline(1,color="green",ls="--");ax.set_ylabel("Relative UA (-)");ax.set_xlabel("Per-HX throughput band");ax.set_title(f"{hx} — relative UA by throughput band");_save(fig,d/"04_relative_ua_by_throughput.png")
    fig,ax=plt.subplots(figsize=(8,5));ax.bar(periods.period,periods.median_relative_ua_empirical);ax.axhline(1,color="green",ls="--");ax.set_ylabel("Median relative UA (-)");ax.tick_params(axis='x',rotation=15);ax.set_title(f"{hx} — pre/reference/later comparison");_save(fig,d/"05_period_comparison.png")
    fig,ax=plt.subplots(figsize=(8,5));pivot=sens.pivot(index="shift_start_days",columns="shift_end_days",values="reference_ua_empirical");im=ax.imshow(pivot,aspect="auto",cmap="viridis");ax.set_xticks(range(len(pivot.columns)),pivot.columns);ax.set_yticks(range(len(pivot.index)),pivot.index);ax.set_xlabel("End-date shift (days)");ax.set_ylabel("Start-date shift (days)");ax.set_title(f"{hx} — empirical reference UA sensitivity (kW/K)");fig.colorbar(im,ax=ax);_save(fig,d/"06_reference_sensitivity.png")
...
def main():
    ...
    pd.concat(frames,ignore_index=True).to_csv(table/"relative_performance_timeseries.csv",index=False);summary_frame=pd.DataFrame(summaries);summary_frame.to_csv(table/"exploratory_summary.csv",index=False);pd.concat(rels).to_csv(table/"operating_relationships.csv",index=False);pd.concat(bands).to_csv(table/"throughput_band_distribution.csv",index=False);pd.concat(periods).to_csv(table/"period_comparison.csv",index=False);pd.concat(sensitivities).to_csv(table/"reference_sensitivity.csv",index=False)
    latest=pd.concat(frames).sort_values("timestamp").groupby("hx_id").tail(1);fig_compare,ax=plt.subplots(figsize=(8,5));ax.bar(latest.hx_id,latest.relative_performance_loss);ax.axhline(0,color="black",ls="--");ax.set(xlabel="HX",ylabel="Latest relative performance loss (-)",title="Cross-HX empirical relative-condition comparison");fig_compare.tight_layout();fig_compare.savefig(fig/"relative_condition_comparison.png",dpi=140);plt.close(fig_compare)
    print(pd.DataFrame(summaries)[["hx_id","reference_ua_empirical","valid_record_count","records_above_reference_pct","impact_status"]].to_string(index=False))
