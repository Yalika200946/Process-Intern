...
def main():
    ...
    shutil.copyfile(BASE/"mix_node_regime_review/mix_node_regime_by_year.csv",OUT/"mix_node_regime_by_year.csv")
    shutil.copyfile(BASE/"mix_node_regime_review/engineering_threshold_proposal.csv",OUT/"engineering_threshold_proposal.csv")
    counter=read(BASE/"pilot_counterfactual/counterfactual_cit_blocker.csv");counter["network_status"]=network["network_status"];counter["cit_recovery_c"]=None;counter["compensation_ratio"]=None;write(counter,"pilot_counterfactual_blocker.csv")
    endpoint=read(BASE/"pilot_counterfactual/pilot_counterfactual_summary.csv");write(endpoint,"pilot_endpoint_counterfactual_summary.csv")
    ...
    thresholds=read(BASE/"mix_node_regime_review/engineering_threshold_proposal.csv")
    payloads={"data_quality":{"status":"VALIDATED","source":"coverage_completion"},"hx_performance":{"status":"PROVISIONAL","records":hx.to_dict("records")},"experimental_analytics":{"status":"EXPLORATORY","banner":banner,"network_status":network,"mix_metrics":mix.to_dict("records"),"gates":gates.to_dict("records"),"threshold_proposals":thresholds.to_dict("records"),"pilot_endpoint_counterfactual":endpoint.to_dict("records"),"counterfactual_cit_status":counter.to_dict("records")}}
    hashes={}
