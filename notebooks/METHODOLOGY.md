...
- `notebooks/bypass_config.py` parse `list bypass Cleaning Heat Exchanger.xlsx` (ต่อ shell → รวมเป็นกลุ่ม HX):
...
- **ΔCIT ใช้ค่าวัดจริง** (median จาก audit) ก่อนเสมอ — ค่าโมเดล (2d/6d) over-estimate ~3× (E113A: โมเดล 6.1° vs วัดจริง/โรงงาน ~2°).
...
### 7.5 TAM deep analysis (`4_tam_deep_analysis.ipynb` → `tam_analysis.json`)
...
**ปัญหาเดิม (ยืนยันด้วยข้อมูล):** `2_Feature_calculation.ipynb` fit OLS เชิงเส้นต่อรอบบน U_relative ตรง ๆ →
...
`SWAP_CAPABLE`/`TAM_ONLY` ที่เขียนมือซ้ำกัน 2 จุด (`2d_cleaning_priority_ranking.ipynb`, `3b_time_to_clean_prediction.ipynb`)
