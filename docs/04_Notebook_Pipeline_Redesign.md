...
Status: **IN PROGRESS — Notebooks 01-03 built and verified against real data (2026-07-16); 04-13 not yet built**
...
| 02 Data Ingestion & Tag Mapping | How do we get a clean, merged process+crude-assay table? | `notebooks/01_data_cleaning.ipynb` (ingestion half) + `00_data_prep_crude_assay.ipynb` | ✅ **Built & verified** (2026-07-16) — ported TAM/shutdown-removal + crude-assay-merge logic verbatim; output row count (1,898) matches the legacy pipeline's known snapshot exactly; column count matches (97 process + 6 crude). Reads `data_profile.json`/`data_dictionary.csv` from Notebook 01 rather than re-deriving them. TAM thresholds + crude assay path centralized into `src/cpht/config.py` (fixes the Phase 6 "scattered config" gap from day one instead of deferring it). |
| 03 Data Quality | Is the merged data trustworthy enough to compute on? | `notebooks/01_data_cleaning.ipynb` (outlier/chain-consistency half) + `notebooks/nb_audit.py`'s `data_quality_report` | ✅ **Built & verified** (2026-07-16) — ported chain-consistency/rolling-z-score/physical-bounds outlier correction verbatim (19 cold-side temp tags, 989 readings flagged/filled). **New: Data Quality Score (FR-DQ-012)** built per HX (unweighted mean of completeness/validity/consistency, documented as a new design choice not an industry standard) — closes the gap the legacy pipeline never addressed. Mean score 0.986, lowest E106AB at 0.935, all 16 HX scored (E101G correctly excluded — no tags). Known gaps: flatline detection (FR-DQ-004) not implemented, flow-tag validity not scored (no physical-bounds definition exists for flow in the ported logic) — documented in the notebook's Limitations, not silently dropped. |
