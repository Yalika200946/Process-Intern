...
```
- Executed notebook copies are written to `reports/executed_notebooks/`; source files under `notebooks/` are not modified in place.
- Set `CPHT_DATA_DIR` to use a data directory other than the local Windows default. The same value is inherited by every pipeline subprocess.
...
| `src/reporting/dashboard_topology.py` (moved 2026-07-17, was `notebooks/build_dashboard_topology.py`) | สร้าง `pfd_topology.json` จาก `src.domain.config` |
| `notebooks/add_forecast_intervals.py` | เพิ่มช่วงความเชื่อมั่นให้ forecast (superseded — ไม่ถูกเรียกใน `run_all.py` แล้ว ดู module docstring) |
| `pipeline/phm_analysis.py` + `src/models/phm_config.py` (moved 2026-07-17, was `notebooks/phm_config.py`) | PHM: RUL (Monte-Carlo), Weibull survival/hazard, propagation model-compare, degradation drivers → `rul/reliability/propagation_models/drivers.json` |
